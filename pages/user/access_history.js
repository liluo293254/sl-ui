...
Page({
  ...
  getShopList(){
    ...
    wx.request({
      url: apiAddr.get_access_history_shop_list,
      method: 'post',
      header: {
        'Content-type': 'application/x-www-form-urlencoded'
      },
  ...
  shopDel: function(event){
    let id = event.currentTarget.dataset.id;
    let index = event.currentTarget.dataset.index;
    let _that = this;
    let shopList = _that.data.shopList;
    wx.request({
      url: apiAddr.access_history_shop_del,
      method: 'post',
      header: { 'Content-type': 'application/x-www-form-urlencoded' },
      data: {user_id:_that.data.user_id,shop_id:id},
      dataType: 'json',
      success: (result) => {  
        ...
        if(data.code == 200){
          ...
          setData(_that,'shopList', shopList); 
          
        }
        if(shopList.length<= 0){
          setData(_that,'showNone',1); 
        }
      }, 
    });
  },
   ...
   */
  onShow() {
    console.log(this.data.old_area_id);
