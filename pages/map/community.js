...
Page({
  ...
  getShopList(selectType, searchKeyWord){
    ...
    wx.request({
      ...
      success: (result) => {
        ...
        wx.hideLoading()
        console.log(result);
        let { data: { map_community_list } } = result;
        if ( map_community_list ) {
          console.log(map_community_list);
          setData(this, 'mapList', map_community_list);
          setData(this, 'mapMarker', this.setMarker(map_community_list));
        }
      },
  ...
  linkWebView(event){
    ...
    let navList = ['typeDetail','communityDetail'];

    if (mapMarkerId > -1 && navList.indexOf(link) >= 0) {
      let id = mapList[mapMarkerId].community_id || mapList[mapMarkerId].shop_id;
      let paramsUrl = {
        communityDetail:  `${baseUrl}/wap/community/detail/community_id/${id}`,
        shopMall:    `${baseUrl}/wap/mall/index/shop_id/${id}`,
        shopWorker:  `${baseUrl}/wap/appoint/worker_list/shop_id/${id}`,
        shopBooking: `${baseUrl}/wap/shop/detail/shop_id/${id}`,
        typeDetail:  `${baseUrl}/wap/shop/detail/shop_id/${id}`
      };
